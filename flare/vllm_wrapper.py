...
def make_context(
    ...
    if chat_format == "continue-chatml":
        im_start, im_end = "<|im_start|>", "<|im_end|>"
        im_start_tokens = [tokenizer.im_start_id]
        im_end_tokens = [tokenizer.im_end_id]
        nl_tokens = tokenizer.encode("\n")
        ...
        context_tokens += (
            ...
            + tokenizer.encode("assistant")
            + nl_tokens
            + tokenizer.encode(history, allowed_special=set())
        )
        raw_text += f"\n{im_start}user\n{query}{im_end}\n{im_start}assistant\n{history}"
        # print(raw_text)
    ...
    def chat(self,
        ...
        **kwargs):
        ...
        req_outputs = self.model.generate([query],
                                            ...
                                            prompt_token_ids=[context_tokens], use_tqdm=False)
        ...
        if chat_format == "continue-chatml":
            return response, [(prompt_str, response)]
