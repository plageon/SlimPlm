...
def chat_prompt_search(data_line, chat_model):
    ...
    if "llama" in chat_model and "tiny" not in chat_model:
        prompt = llama_rag_prompt.format(sys_prompt=sys_prompt, demo=demo, references=context, question=question)
        ...
        prompt = qwen_rag_prompt.format(sys_prompt=sys_prompt, demo=demo, references=context, question=question)
        while len(tokenizer.encode(prompt)) > 3000:
            #  remove the last 2 sentences
            _context = _context[:-2]
            context = "\n".join(_context)
...
if __name__ == '__main__':
    ...
    input_file = f"./user_intent_data/{dataset}/{chat_model}/selfask/{chat_model}round{round}-{dataset}-{split}.jsonl"
    output_file = f"./user_intent_data/{dataset}/{chat_model}/selfask/{chat_model}round{round}intans-{dataset}-{split}.jsonl"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        for idx, data_line in tqdm(enumerate(data_lines), total=len(data_lines),
            ...
            if "search_results" not in data_line or len(data_line["search_results"]) < round or len(
                    data_line["search_results"][round]) == 0:
                continue
            ...
            if f"{chat_model}_intermediate_answer" not in data_lines[idx]:
                data_lines[idx][f"{chat_model}_intermediate_answer"] = []
            data_lines[idx][f"{chat_model}_intermediate_answer"].append(res)
